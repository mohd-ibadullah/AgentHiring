...
if __name__ == "__main__":
    ...
    st.set_page_config(
        page_title="AgentHiring — Candidate Discovery & Ranking",
        page_icon="🔍",
        layout="wide",
    ...
    # App layout
    st.markdown('<div class="main-title">AgentHiring</div>', unsafe_allow_html=True)
    ...
    tab_rank, tab_agent = st.tabs(["🏆 Candidate Discovery & Ranking", "💬 Concierge Recruiting Agent"])
    ...
    with tab_agent:
        st.markdown('<h3 style="margin-top: 0px;">💬 Concierge Recruiting Agent Chat</h3>', unsafe_allow_html=True)
        st.markdown(
            "Welcome! I am your AI Recruiting Agent. You can query candidate details, compare matches, "
            "ask why a candidate scored a certain rank, probe for honeypot traps, or request interview screening questions."
        )

        # Show configuration warning if GEMINI_API_KEY is missing
        api_key = get_api_key()
        if not api_key:
            st.warning("⚠️ **GEMINI_API_KEY** not detected in environment or `.env` file. "
                       "The agent will run in local rule-based concierge mode for demonstration. "
                       "Configure a Gemini API Key to enable full interactive conversations.")
        ...
        # Handle user input
        if chat_input := st.chat_input("Ask me about candidates (e.g., 'Compare CAND_0000010 and CAND_0000012', 'Is CAND_0000002 a honeypot?'):"):
            ...
            # Run ADK agent
            with st.spinner("Agent is analyzing profiles and executing tools..."):
                try:
                    from src.agent import run_agent
                    # Execute agent query
                    response = run_agent(chat_input)
                except Exception as e:
